        ...
        render: function () {
            var p = this.getPosition(),
                ...
                dropdown = '<div class="city-picker-dropdown" style="left:0px;top:100%;' +
                    ...
                    '<a class="active" data-count="province" id="select-province">省份</a>' +
                    (this.includeDem('city') ? '<a data-count="city">城市</a>' : '') +
                    (this.includeDem('district') ? '<a data-count="district">区县</a>' : '') + '</div>' +
                    '<div class="city-select-content">' +
                    '<div class="city-select province" data-count="province" id="select-province"></div>' +
                    (this.includeDem('city') ? '<div class="city-select city" data-count="city" id="select-city"></div>' : '') +
                    (this.includeDem('district') ? '<div class="city-select district" data-count="district" id="select-district"></div>' : '') +
                    '</div></div>';
        ...
        getText: function () {
            ...
            this.$dropdown.find('.city-select')
                .each(function () {
                    ...
                    var id= $(this).attr("id");
                    if (item) {
                        text += ($(this).hasClass('province') ? '' : '/') + '<span id="' + id + '" class="select-item" data-count="' +
                ...
                $.ajax({
                    ...
                    success : function(data) {
                        if(data.code == Codes.successResponse && !checkDataEmpty(data.result)){
                            areaMap.setZoomAndCenter(18, [data.result.centerX,data.result.centerY]);
                            $("#add-area-select-code").val(data.result.code);
                            $("#add-area-modal-cityId").val(data.result.id);
                            $("#add-area-modal-name").val(data.result.name + "停车点");
                        }
                    }
                });
                this.unbind();
