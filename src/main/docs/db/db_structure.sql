...
(
  ...
  comment '用户申请表'  engine=InnoDB charset=utf8
;
...
(
  id int not null auto_increment comment '车辆编号'
    primary key,
...
create index start_city
  on journey (start_area)
;

create index end_city
  on journey (end_area)
;

create index ride_time
  on journey (ride_time)
;
...
(
  id int not null auto_increment comment '记录编号'
    primary key,
  user_id char(11) null comment '用户编号',
  vip_time int null comment '充值的月数',
  amount decimal(6,2) null comment '花费金额',
  operate_time varchar(50) null comment '操作时间'
)
  comment '包月充值记录表'  engine=InnoDB charset=utf8
;